...
Scans the I²C bus for a Diablo boards and returns a list of all usable addresses
...
Scans the I²C bus for the first Diablo and sets it to a new I2C address
...
busNumber               I²C bus on which the Diablo is attached (Rev 1 is bus 0, Rev 2 is bus 1)
bus                     the smbus object used to talk to the I²C bus
i2cAddress              The I²C address of the Diablo chip to control
...
printFunction           Function reference to call when printing text, if None "print" is used
    ...
    i2cAddress              = I2C_ID_DIABLO  # I²C address, override for a different address
    bus                     = None
    foundChip               = False
    printFunction           = None
...
If tryOtherBus is True or omitted, this function will attempt to use the other bus if the Diablo devices can not be found on the current busNumber
        """
...
If False all of the motor pins are tri-stated (unpowered)
        ...
        try:
            self.bus.write_byte_data(self.i2cAddress, COMMAND_SET_ENABLED, level)
        except KeyboardInterrupt:
            raise
        except:
...
If False all of the motor pins are tri-stated (unpowered)
        """ 
        try:
            i2cRecv = self.bus.read_i2c_block_data(self.i2cAddress, COMMAND_GET_ENABLED, I2C_MAX_LEN)
        except KeyboardInterrupt:
            raise
        except:
