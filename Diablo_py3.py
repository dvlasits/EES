...
"""
This module is designed to communicate with the Diablo

Use by creating an instance of the class, call the Init function, then command as desired, e.g.
import Diablo_py3
DIABLO = Diablo_py3.Diablo()
DIABLO.Init()
# User code here, use DIABLO to control the board

Multiple boards can be used when configured with different I2C addresses by creating multiple instances, e.g.
import Diablo_py3
DIABLO1 = Diablo_py3.Diablo()
DIABLO2 = Diablo_py3.Diablo()
DIABLO1.i2cAddress = 0x44
DIABLO2.i2cAddress = 0x45
DIABLO1.Init()
DIABLO2.Init()
# User code here, use DIABLO1 and DIABLO2 to control each board separately

For explanations of the functions available call the Help function, e.g.
import Diablo_py3
DIABLO = Diablo_py3.Diablo()
DIABLO.Help()
See the website at www.piborg.org/diablo for more details
"""
...
COMMAND_SET_ENABLED     = 30    # Set if the motor drives are enabled
COMMAND_GET_ENABLED     = 31    # Get the motor drive enabled state
...
Scans the I2C bus for a Diablo boards and returns a list of all usable addresses
...
Scans the I2C bus for the first Diablo and sets it to a new I2C address
...
Warning, this new I2C address will still be used after resetting the power on the device
    ...
    if oldAddress < 0x0:
        ...
        if len(found) < 1:
            print('No Diablo boards found, cannot set a new I2C address!')
            return
        else:
            oldAddress = found[0]
...
busNumber               I2C bus on which the Diablo is attached (Rev 1 is bus 0, Rev 2 is bus 1)
bus                     the smbus object used to talk to the I2C bus
i2cAddress              The I2C address of the Diablo chip to control
foundChip               True if the Diablo chip can be seen, False otherwise
printFunction           Function reference to call when printing text, if None "print" is used
    ...
    i2cAddress              = I2C_ID_DIABLO  # I2C address, override for a different address
    foundChip               = False
    printFunction           = None
    i2cWrite                = None
    i2cRead                 = None
...
Sends a raw command on the I2C bus to the Diablo
Command codes can be found at the top of Diablo.py, data is a list of 0 or more byte values
...
Reads data back from the Diablo after sending a GET command
Command codes can be found at the top of Diablo.py, length is the number of bytes to read back
...
Prepare the I2C driver for talking to a Diablo on the specified bus and I2C address
...
DIABLO.printFunction = DIABLO.NoPrint
        """
        pass
...
Prepare the I2C driver for talking to the Diablo

If tryOtherBus is True, this function will attempt to use the other bus if the Diablo devices can not be found on the current busNumber
    This is only really useful for early Raspberry Pi models!
        """
        self.Print('Loading Diablo on bus %d, address %02X' % (self.busNumber, self.i2cAddress))
        ...
        try:
            ...
            if len(i2cRecv) == I2C_MAX_LEN:
                if i2cRecv[1] == I2C_ID_DIABLO:
                    self.foundChip = True
                    self.Print('Found Diablo at %02X' % (self.i2cAddress))
                else:
                    self.foundChip = False
                    self.Print('Found a device at %02X, but it is not a Diablo (ID %02X instead of %02X)' % (self.i2cAddress, i2cRecv[1], I2C_ID_DIABLO))
            else:
                self.foundChip = False
                self.Print('Missing Diablo at %02X' % (self.i2cAddress))
        except KeyboardInterrupt:
            raise
        except:
            self.foundChip = False
        ...
        if not self.foundChip:
            self.Print('Diablo was not found')
            if tryOtherBus:
                if self.busNumber == 1:
                    self.busNumber = 0
                else:
                ...
                self.Print('Are you sure your Diablo is properly attached, the correct address is used, and the I2C drivers are running?')
                self.bus = None
        else:
            self.Print('Diablo loaded on bus %d' % (self.busNumber))
